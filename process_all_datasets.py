...
def process_dataset(dataset_name, is_debug=True):
    ...
    total_operations += 1
    cmd = f"python pa1.py --data_dir '{data_dir}' --output_dir '{output_dir}' --name_3 {dataset_name}-empivot --output_file1 {dataset_name}_EM_pivot"
    if run_command(cmd, f"Generate EM pivot calibration for {dataset_name}"):
        success_count += 1
    
    # 4. Generate Optical pivot calibration
    total_operations += 1
    cmd = f"python pa1.py --data_dir '{data_dir}' --output_dir '{output_dir}' --name {dataset_name}-calbody --name_4 {dataset_name}-optpivot --output_file2 {dataset_name}_Optpivot"
    if run_command(cmd, f"Generate Optical pivot calibration for {dataset_name}"):
        success_count += 1
    
    # 5. Generate final output1 file (for all datasets)
    total_operations += 1
    ...
    if run_command(cmd, f"Generate final output1 for {dataset_name}"):
        ...
        # Compare with expected output if it exists (debug datasets only have expected output)
        if is_debug:
